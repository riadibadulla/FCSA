...
' can', ' castle', ' caterpillar', ' cattle', ' chair', ' chimpanzee', ' clock', ' cloud', ' cockroach', ' couch',
' cra', ' crocodile', ' cup', ' dinosaur', ' dolphin',
...
def train():
    ...
    for epoch in range(0,100):  # loop over the dataset multiple times
        ...
        sch.step()
        print(
            f'Epoch{epoch}:      loss: {running_loss:.3f} accuaracy: {running_accuarcy}% val_loss: {val_loss:.3f} val_acc: {val_acc}%\n')
...
#net.load_state_dict(torch.load("fnet.pth"))
net.to(device)
